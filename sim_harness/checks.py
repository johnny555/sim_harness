...
Service checks, sensor validation, timing, and vehicle motion checks.
...
from sim_harness.spin import (
    spin_for_duration, spin_until_condition,
...
def _acquire_executor(node: Node, executor=None):
    """Determine whether we need a managed executor or can rely on an existing one.

    Returns (executor_or_None, managed: bool).  When *managed* is True the
    caller must call ``executor.remove_node(node)`` in a finally block.
    When False the node is already being spun externally and callers should
    use sleep-based waiting.
    """
    ...
    try:
        ...
        # Node already belongs to another executor (e.g. the fixture's
        # background-spinning MultiThreadedExecutor).  Just wait via sleep.
        return None, False
    return exc, True
...
def check_parameter_exists(
    ...
    try:
        ...
        try:
            ...
            if managed:
                spin_until_condition(exc, future.done, timeout_sec)
            else:
                wait_until_condition(future.done, timeout_sec)
            if not future.done():
                return False
            resp = future.result()
            if not resp.values:
...
def check_publish_rate(
    ...
    try:
        if managed:
            spin_for_duration(exc, sample_duration_sec)
        else:
            wait_for_duration(sample_duration_sec)
    finally:
        node.destroy_subscription(sub)
        if managed:
            exc.remove_node(node)
...
def check_action_server_responsive(
    ...
    try:
        ...
        if managed:
            spin_until_condition(exc, future.done, max_response_time_ms / 1000)
        else:
            wait_until_condition(future.done, max_response_time_ms / 1000)
